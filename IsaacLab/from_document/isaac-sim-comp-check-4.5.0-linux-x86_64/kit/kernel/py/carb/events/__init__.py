...
def get_events_adapter_interface() -> IEvents:
    ...
    if not hasattr(get_events_adapter_interface, "iface"):
        get_events_adapter_interface.iface = acquire_events_adapter_interface()
    return get_events_adapter_interface.iface
...
async def _next_event(self: IEventStream, order: int = 0, name: str = ""):
    ...
    if len(name) == 0:
        ...
        name = f"{frame.f_code.co_name} at {frame.f_code.co_filename}:{frame.f_lineno}"
