...
def get_all_sys_paths() -> Iterator[str]:
    ...
    """
    yield from sys.path
    yield from get_fast_importer_sys_paths()
...
def _refresh_cache():
    ...
    old_paths = _cache.known_paths
    _cache.known_paths = paths

    for key in _sys_paths.keys():
        if key in old_paths:
            continue
        ...
        for fullpath, subpath in _fast_walk(path, filterdirs):
            ...
            for module_ext in MODULES_EXTS:
                if subpath.endswith(module_ext):
                    ...
                    # Store found python module if it's not already in the cache
                    if module_name in _cache.modules:
                        # This module is already in the cache. It can happen if we have 2 paths to the same module. The first in sys.path
                        # is prioritized, to match what vanilla import is doing.
                        continue
...
class FastFinder:
    ...
    def find_distributions(cls, context):
        ...
        if module_path:
            ...
            search_pattern = pip_install_dir.joinpath(f"{context.name}-*.dist-info").__str__()
