...
class ExtensionModules:
    ...
    def shutdown(self):
        ...
        # WAR The following code is very slow, O(n^2) across all modules on exit. For now we will simply avoid this
        # code on exit, but ideally we would make it so that `find_modules_in_folder` is O(1) in terms of the number
        # of extensions or modules, and therefore it would be safe to execute this block of logic. OVCC-1252 to fix
