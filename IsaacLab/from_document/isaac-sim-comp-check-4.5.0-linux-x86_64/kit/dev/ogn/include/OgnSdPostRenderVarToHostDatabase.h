...
namespace OgnSdPostRenderVarToHostAttributes
{
namespace inputs
{
...
using renderVarHostSuffix_t = const char*&;
ogn::AttributeInitializer<const char*, ogn::kOgnInput> renderVarHostSuffix("inputs:renderVarHostSuffix", "string", kExtendedAttributeType_Regular, "host", 4);
...
using namespace OgnSdPostRenderVarToHostAttributes;
namespace IOgnSdPostRenderVarToHost
{
// Expose a host renderVar from the input device renderVar.
class OgnSdPostRenderVarToHostDatabase : public omni::graph::core::ogn::OmniGraphDatabase
{
public:
    template <typename StateInformation>
    CARB_DEPRECATED("sInternalState is deprecated. Use sSharedState or sPerInstanceState instead")
    static StateInformation& sInternalState(const NodeObj& nodeObj, InstanceIndex index = {kAuthoringGraphIndex}) {
        return sm_stateManagerOgnSdPostRenderVarToHost.getState<StateInformation>(nodeObj.nodeHandle, index);
    }
    template <typename StateInformation>
    static StateInformation& sSharedState(const NodeObj& nodeObj) {
        return sm_stateManagerOgnSdPostRenderVarToHost.getState<StateInformation>(nodeObj.nodeHandle, kAuthoringGraphIndex);
    }
    template <typename StateInformation>
    ...
    static StateInformation& sPerInstanceState(const NodeObj& nodeObj, GraphInstanceID instanceId) {
        return sm_stateManagerOgnSdPostRenderVarToHost.getState<StateInformation>(nodeObj.nodeHandle, instanceId);
    }
    template <typename StateInformation>
    ...
    static ogn::StateManager sm_stateManagerOgnSdPostRenderVarToHost;
    static std::tuple<int, int, int>sm_generatorVersionOgnSdPostRenderVarToHost;
    static std::tuple<int, int, int>sm_targetVersionOgnSdPostRenderVarToHost;
    ...
    struct inputsT {
        ...
        , gpu{offset}
        , renderVar{offset}
        , renderVarHostSuffix{offset,AttributeRole::eText}
        , rp{offset}
        {}
        ...
        ogn::ArrayInput<const char,ogn::kCpu> renderVarHostSuffix;
    ...
    OgnSdPostRenderVarToHostDatabase(NodeObj const& nodeObjParam)
    : OmniGraphDatabase()
    ...
    OgnSdPostRenderVarToHostDatabase(GraphContextObj const&, NodeObj const& nodeObjParam)
    : OgnSdPostRenderVarToHostDatabase(nodeObjParam)
    {}

    //Main constructor
    OgnSdPostRenderVarToHostDatabase(GraphContextObj const* contextObjParam, NodeObj const* nodeObjParam, size_t handleCount)
    : OmniGraphDatabase()
    , inputs{m_offset.index}
    , outputs{m_offset.index}
    {
        ...
        NodeObj const& nodeObj = abi_node();
        {
            auto inputDataHandles0 = getAttributesR<
                ...
                >(contextObj, nodeObj.nodeContextHandle, std::make_tuple(
                    inputs::exec.m_token, inputs::gpu.m_token, inputs::renderVar.m_token, inputs::renderVarHostSuffix.m_token,
                    inputs::rp.m_token
                )
            ...
            inputs.renderVarHostSuffix.setContext(contextObj);
            inputs.renderVarHostSuffix.setHandle(std::get<3>(inputDataHandles0));
...
public:
    static void initializeType(const NodeTypeObj& nodeTypeObj)
    {
        ...
        if( ! iTokenPtr ) {
            CARB_LOG_ERROR("IToken not found when initializing omni.syntheticdata.SdPostRenderVarToHost");
            return;
        }
        ...
        inputs::renderVarHostSuffix.initialize(iToken, *iNodeType, nodeTypeObj);
        ...
        iNodeType->setMetadata(nodeTypeObj, kOgnMetadataDescription, "Expose a host renderVar from the input device renderVar.");
    }
    static void initialize(const GraphContextObj&, const NodeObj& nodeObj)
    {
        ...
        attr = iNode->getAttributeByToken(nodeObj, inputs::renderVarHostSuffix.token());
        attr.iAttribute->setMetadata(attr, kOgnMetadataDescription, "Suffix appended to the renderVar name");
        attr.iAttribute->setMetadata(attr, kOgnMetadataDefault, "\"host\"");
    ...
    {
        sm_stateManagerOgnSdPostRenderVarToHost.removeState(nodeObj.nodeHandle, instanceID);
    }
    bool validate() const {
        return validateNode()
            && inputs.exec.isValid()
            ...
            && inputs.renderVarHostSuffix.isValid()
            && inputs.rp.isValid()
    ...
    void preCompute() {
        if(m_canCachePointers == false) {
            ...
            inputs.renderVarHostSuffix.invalidateCachedPointer();
        ...
        for(NameToken const& attrib : m_mappedAttributes) {
            ...
            if(attrib == inputs::renderVarHostSuffix.m_token) {
                inputs.renderVarHostSuffix.invalidateCachedPointer();
                continue;
            }
    ...
    void onDataLocationChanged(AttributeObj const& attr) {
        ...
        if(token == inputs::renderVarHostSuffix.m_token) {
            ConstAttributeDataHandle hdl = attr.iAttribute->getConstAttributeDataHandle(attr, m_offset);
            inputs.renderVarHostSuffix.setHandle(hdl);
            return;
        }
    ...
    omni::graph::image::unstable::ComputeParamsBuilder<std::string> getCudaParamsBuilder(InstanceIndex relativeIdx = { 0 }) {
        auto variable = const_cast<OgnSdPostRenderVarToHostDatabase*>(this)->getVariable("__rpInteropState", m_offset + relativeIdx);
    ...
    void setCudaParams(omni::graph::image::unstable::ComputeParams<std::string>&& params, InstanceIndex relativeIdx = { 0 }) {
        auto variable = const_cast<OgnSdPostRenderVarToHostDatabase*>(this)->getVariable("__computeParams", m_offset + relativeIdx);
    ...
    cudaStream_t getCudaStream(InstanceIndex relativeIdx = { 0 }) const {
        auto variable = const_cast<OgnSdPostRenderVarToHostDatabase*>(this)->getVariable("__cudaStream", m_offset + relativeIdx);
        return variable.isValid() ? reinterpret_cast<cudaStream_t>(*variable.get<uint64_t>()) : cudaStream_t{};
    }
};
ogn::StateManager OgnSdPostRenderVarToHostDatabase::sm_stateManagerOgnSdPostRenderVarToHost;
std::tuple<int, int, int> OgnSdPostRenderVarToHostDatabase::sm_generatorVersionOgnSdPostRenderVarToHost{std::make_tuple(1,79,1)};
std::tuple<int, int, int> OgnSdPostRenderVarToHostDatabase::sm_targetVersionOgnSdPostRenderVarToHost{std::make_tuple(2,181,8)};
}
using namespace IOgnSdPostRenderVarToHost;
#define REGISTER_OGN_NODE() \
namespace { \
    ogn::NodeTypeBootstrapImpl<OgnSdPostRenderVarToHost, OgnSdPostRenderVarToHostDatabase> s_registration("omni.syntheticdata.SdPostRenderVarToHost", 1, "omni.syntheticdata"); \
}
