...
def remove_generated_part_from_kit_file(path):
...
def get_all_extensions(ext_folders):
    """Find all extensions in the repo"""
...
def generate_exts_deps_app(generated_app_path, local_extensions):
    renegenerate = True
    if os.path.exists(generated_app_path):
        ...
        if ext_deps == set(local_extensions):
            renegenerate = False
        else:
...
def run(tool_config, options, root, platform_host):
    ...
    local_extensions = list(get_all_extensions(ext_folders))
    ...
    def get_all_apps():
        ...
        if generated_app_path and len(local_extensions) > 0:
            ...
            yield generate_exts_deps_app(generated_app_path, local_extensions), extra_args
    ...
    for app_path, extra_args in get_all_apps():
        ...
        is_updating_lock = options.update or options.clean
        if is_updating_lock:
            remove_generated_part_from_kit_file(app_path)
...
def setup_repo_tool(parser: argparse.ArgumentParser, config: Dict) -> Callable:
    ...
    parser.description = "Tool to precache kit apps. Downloads extensions without running."
    ...
    parser.add_argument(
        ...
        required=False,
        action="store_true",
        help="Update version lock: remove generated part from kit files, clean cache path, then run ext precaching.",
    )
