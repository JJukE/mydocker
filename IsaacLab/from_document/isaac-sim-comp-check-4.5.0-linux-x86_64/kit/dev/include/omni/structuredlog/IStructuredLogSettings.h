...
protected:
     ...
     *
     *  @returns The current event identifier mode.  The default is @ref omni::structuredlog::IdMode::eFastSequential.
     *
     *  @thread_safety This call is thread safe.
     */
     ...
     *
     *  @remarks Once the event queue is running (ie: the first message has been sent), changing
     *           its size will cause a second queue to be created at the new size.  The old queue
     *           will be flushed and the new queue will be made active.  There will be a short
     *           period where two queues will be running simultaneously.  Any new events that
     *           are queued during this period will be added to the new queue.  There is no need
     *           to stop the old queue before changing the size, this will be handled
     *           automatically.  Note that during one of these queue transitions there may be
     *           a short period where omni::structuredlog::getEventQueueSize() may return the
     *           new queue's size even if the old queue is still active.
     *
     *  @note Changing the queue size frequently will lead to stalls in the processing pipeline.
     *        Generally the queue size should be set once on startup of an app and left at that
     *        configured size for the process' lifetime.
     *
     *  @thread_safety This call is thread safe.
     */
     ...
     *                      an empty string or nullptr to restore the default behavior of writing
     *                      each new message to its own schema's log file.  This should just be
     *                      the name of the file and not include any path components.  This may
     *                      also be one of the special values `/dev/stdout` or `/dev/stderr` to
     *                      send the output to only the stdout or stderr streams.  In this case,
     *                      all events are output to these streams even if a given schema uses
     *                      the `fSchemaFlagUseLocalLog` or event uses the `fEventFlagUseLocalLog`
     *                      flag.  Otherwise, if a filename is given and this name contains any
     *                      path components (ie: a '/'), setting the default log name will fail.
     *                      The log output path should be set using
     ...
     *                 ID string returned from @ref omni::structuredlog::IStructuredLogSettings::getUserId()
     *                 and that nothing else has cached that returned string since it could become
     *                 invalidated here.
     */
...
public:
     ...
     *  @param[in] flags    Flags to affect the behavior of this operation.  This must be
