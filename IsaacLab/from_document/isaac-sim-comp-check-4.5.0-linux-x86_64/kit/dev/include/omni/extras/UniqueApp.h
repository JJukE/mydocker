...
public:
     ...
     *
     *  @param[inout] other The other object to move data from.
     *
     *  @remarks This moves the information from another object into this one.  The other
     *           object will still be valid but will have been reset to an empty state.
     */
     ...
     *  @remarks This moves the information from another object into this one.  The other
     *           object will still be valid but will have been reset to an empty state.
     *           Upon return, this object will only be as valid as the other object was.
     */
...
#    if CARB_TOOLCHAIN_CLANG
    static constexpr FileHandle kBadFileHandle =
        __builtin_constant_p(CARBWIN_INVALID_HANDLE_VALUE) ? CARBWIN_INVALID_HANDLE_VALUE : CARBWIN_INVALID_HANDLE_VALUE;
#    else
...
#    endif
    ...
    static bool _lockFile(FileHandle fp, LockType type, LockAction action = LockAction::eSet)
    {
        ...
        // attempt to lock the file.  We'll use the non-waiting version of the lock so that we
        // can simply fail back to the caller if the lock can't be acquired at the moment.
        // We'll also use the newer "open file descriptor file locking" API since that stores
        // the lock records at the file descriptor level instead of the process level.  The
        // older "advisory file locking" API stores the lock records at the process level and
        // will therefore lose locks if any other thread opens then closes the same file even
        // with another file descriptor.  The older API also has some potential issues with
        // locks getting lost unexpectedly in network file mount situations.
        fl.l_type = (type == LockType::eExclusive ? F_WRLCK : F_RDLCK);
        fl.l_whence = SEEK_SET;
        fl.l_start = 0;
        fl.l_len = 512;
        fl.l_pid = 0;
        result = fcntl(fp, (action == LockAction::eTest ? F_OFD_GETLK : F_OFD_SETLK), &fl);
