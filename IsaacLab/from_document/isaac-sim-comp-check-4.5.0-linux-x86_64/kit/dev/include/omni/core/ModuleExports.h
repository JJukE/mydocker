...
//! Unique type name for @ref omni::core::ModuleExportEntryOnModuleLastChanceShutdown.
constexpr const char* const kModuleExportEntryTypeOnModuleLastChanceShutdown = "omniOnModuleLastChanceShutdown";
...
//!
//! This function will be called during 'quick shutdown' instead of the regular unload path if it
//! is specified.
//!
//! This function will also be called during normal shutdown if a module is unable to unload.
//! If a module returns `false` from @ref OnModuleCanUnloadFn, this will be tried again until there
//! are no more modules that can be unloaded. If the module still returns `false` from @ref OnModuleCanUnloadFn,
//! it is considered to be unable to unload.
//!
//! This function should do any final work that's required before exiting, such as flushing
//! files to disk to avoid data loss or producing some final telemetry events. This function
//! shouldn't try to clean up resources since the 'quick shutdown' path terminates the process at
//! the end. The 'quick shutdown' path is supposed to be quick, so avoid doing unnecessary work in
//! this function.
//!
//! It is safe to access the owning @ref omni::core::ITypeFactory.
//!
//! No other module functions will be called while this function is active.
using OnModuleLastChanceShutdownFn = void();

//! @ref omni::core::ModuleExports entry to register a function to be called when the plugin unload
//! will be skipped during a quick shutdown
//!
//! Use the helper @ref OMNI_MODULE_ON_MODULE_LAST_CHANCE_SHUTDOWN to add this entry.
OMNI_MODULE_EXPORT_ENTRY_BEGIN(ModuleExportEntryOnModuleLastChanceShutdown)
OnModuleLastChanceShutdownFn* onModuleLastChanceShutdown; //!< Module function to call to clean-up the module during
                                                          //!< unload.
OMNI_MODULE_EXPORT_ENTRY_END(ModuleExportEntryOnModuleLastChanceShutdown)

//! Registers the plugin's function who is responsible for cleaning up the plugin when the plugin
//! unload is being skipped during a quick shutdown.
//!
//! This function should do any final work that's required before exiting, such as flushing
//! files to disk to avoid data loss or producing some final telemetry events. This function
//! shouldn't try to clean up resources since the 'quick shutdown' path terminates the process at
...
//! @param exp_ The @ref omni::core::ModuleExports table in which the entry should be added.
//!
//! @param fn_ The plugin's @ref omni::core::OnModuleLastChanceShutdownFn who is responsible for cleaning up the plugin.
#define OMNI_MODULE_ON_MODULE_LAST_CHANCE_SHUTDOWN(exp_, fn_)                                                          \
    OMNI_RETURN_IF_FAILED(exp_->addOnModuleLastChanceShutdown(fn_))
...
std::atomic<carb::profiler::IProfiler*>* profiler; //!< Pointer to the module's @ref g_carbProfiler.
...
struct ModuleExports
{
    ...
    //! See @ref OMNI_MODULE_ON_MODULE_LAST_CHANCE_SHUTDOWN.
    Result addOnModuleLastChanceShutdown(OnModuleLastChanceShutdownFn* fn,
                                         ModuleExportEntryFlag flags = fModuleExportEntryFlagNone)
    {
        ModuleExportEntryOnModuleLastChanceShutdown entry{ kModuleExportEntryTypeOnModuleLastChanceShutdown, flags };
        entry.onModuleLastChanceShutdown = fn;
        return add(reinterpret_cast<ModuleExportEntry*>(&entry));
    }
