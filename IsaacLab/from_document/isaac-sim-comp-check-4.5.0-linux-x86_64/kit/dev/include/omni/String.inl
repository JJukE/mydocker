...
#include "../carb/detail/PushBadMacros.h"
...
// Work around gcc bug 97185 such as:
// ‘void* __builtin_memchr(const void*, int, long unsigned int)’ specified bound 18446744073709551612 exceeds
// maximum object size 9223372036854775807 [-Werror=stringop-overread]
#if CARB_COMPILER_GNUC && __GNUC__ >= 11 && __GNUC__ < 12
#    define CARB_GNUC11_ONLY(...) __VA_ARGS__
#else
#    define CARB_GNUC11_ONLY(...)
#endif
                ...
                Base... base)
{
    ...
    if (endptr == str)
    {
        CARBLOCAL_THROW(std::invalid_argument, name);
    }
    else if (errno == ERANGE || range_check::check(conv_result, std::is_same<Ret, int>{}))
    {
        CARBLOCAL_THROW(std::out_of_range, name);
    }
    else
    {
        result = static_cast<Ret>(conv_result);
    }

    if (pos)
    {
        *pos = (std::size_t)(endptr - str);
    }
...
constexpr void null_check(const char* ptr, const char* function)
{
    if (ptr == nullptr)
    {
        CARBLOCAL_THROW(std::invalid_argument, (std::string(function) + " invalid nullptr argument").c_str());
    }
}
} // namespace detail
...
inline string::string(size_type n, value_type c)
{
    ...
    set_empty();
    traits_type::assign(allocate_if_necessary(new_size), new_size, c);
    set_size(new_size);
}
...
template <typename InputIterator CARB_NO_DOC(
    , std::enable_if_t<carb::cpp::detail::IsLegacyInputIterator<InputIterator>::value, bool>)>
string::string(InputIterator begin, InputIterator end)
{
    set_empty();
    initialize(begin, end, typename std::iterator_traits<InputIterator>::iterator_category{});
}
...
inline string::string(string&& str) noexcept
{
    ...
    if (str.is_local())
    {
        std::memcpy(m_local_data.data(), str.m_local_data.data(), str.size());
        set_size(str.size());
        str.set_size(0);
    }
    else
    {
        m_local_data = str.m_local_data;
        set_allocated();
        str.set_empty();
    }
}
...
{
    set_empty();
    initialize(ilist.begin(), ilist.end(), std::iterator_traits<decltype(ilist.begin())>::iterator_category{});
}
...
inline string::string(const carb::cpp::string_view& sv)
{
    set_empty();
    initialize(sv.data(), sv.size());
}

inline string::string(const carb::cpp::string_view& sv, size_type pos, size_type n)
{
...
string::string(formatted_t, const char* fmt, Args&&... args)
{
    ...
    // returns the number of characters that would be written given unlimited buffer.
    size_type fmt_size = snprintf_check(m_local_data.data(), kSMALL_STRING_SIZE, fmt, std::forward<Args>(args)...);

    length_check(0, fmt_size, "string::string");

    if (fmt_size >= kSMALL_STRING_SIZE)
    {
        auto p = allocate_if_necessary(fmt_size);
        // CARB_SCOPE_EXCEPT would be applicable here, but it is slow (much slower than a `try` block) and on older
        // versions of glibc (especially 2.17 which is what Centos7 uses) it can lock an internal mutex that is shared
        // with the loader, which can cause deadlocks.
        CARBLOCAL_TRY_BEGIN
        {
            // If vsnprintf_check throws (it shouldn't), clean up.
            snprintf_check(p, fmt_size + 1, fmt, std::forward<Args>(args)...);
        }
        CARBLOCAL_CATCH_ALL
        {
            dispose();
...
inline string::string(vformatted_t, const char* fmt, va_list ap)
{
    ...
    size_type fmt_size = vsnprintf_check(m_local_data.data(), kSMALL_STRING_SIZE, fmt, ap);
    ...
    if (fmt_size >= kSMALL_STRING_SIZE)
    {
        ...
        CARBLOCAL_TRY_BEGIN
        {
            vsnprintf_check(p, fmt_size + 1, fmt, ap2);
        }
        CARBLOCAL_CATCH_ALL
        {
            dispose();
...
inline string& string::operator=(string&& str) noexcept
{
    ...
    if (!is_local())
    {
        auto adata = carb::cpp::bit_cast<allocated_data>(m_local_data);
        cur_data = adata.m_ptr;
        cur_capacity = adata.m_capacity;
    }
    ...
    if (cur_data != nullptr)
    {
        auto other_adata = carb::cpp::bit_cast<allocated_data>(str.m_local_data);
        other_adata.m_ptr = cur_data;
        other_adata.m_capacity = cur_capacity;
        str.m_local_data = carb::cpp::bit_cast<local_data>(other_adata);
        str.set_allocated();
    }
...
inline string& string::operator=(const carb::cpp::string_view& sv)
{
...
template <class InputIterator, std::enable_if_t<carb::cpp::detail::IsLegacyInputIterator<InputIterator>::value, bool>>
string& string::assign(InputIterator first, InputIterator last)
{
    return assign_internal(first, last, typename std::iterator_traits<InputIterator>::iterator_category{});
}

inline string& string::assign(std::initializer_list<value_type> ilist)
{
    return assign_internal(
        ilist.begin(), ilist.end(), std::iterator_traits<decltype(ilist.begin())>::iterator_category{});
}
...
inline string& string::assign(const carb::cpp::string_view& sv)
{
    return assign_internal(sv.data(), sv.size());
}

inline string& string::assign(const carb::cpp::string_view& sv, size_type pos, size_type n)
{
...
inline string& string::assign_vprintf(const char* fmt, va_list ap)
{
    ...
    int check = std::vsnprintf(allocate_if_necessary(fmt_size), fmt_size + 1, fmt, ap2);
...
constexpr string::operator carb::cpp::string_view() const noexcept
{
    return carb::cpp::string_view(data(), size());
}
...
constexpr string::size_type string::size() const noexcept
{
    if (is_local())
    {
        size_type const cap = (size_type)(unsigned char)m_local_data.data()[kSMALL_SIZE_OFFSET];
        // GCC needs some assurances about the result, otherwise we get warnings about over-reads.
        CARB_GNUC11_ONLY(CARB_ASSUME(cap < kSMALL_STRING_SIZE);)
        return kSMALL_STRING_SIZE - cap - 1;
    }
    else
    {
        return carb::cpp::bit_cast<allocated_data>(m_local_data).m_size;
    }
}
...
inline void string::reserve(size_type new_cap)
{
    ...
    if (new_cap != capacity())
    {
        ...
        if (new_cap > capacity() || new_cap >= kSMALL_STRING_SIZE)
        {
            ...
            auto adata = carb::cpp::bit_cast<allocated_data>(m_local_data);
            adata.m_ptr = tmp_ptr;
            adata.m_capacity = new_cap;
            adata.m_size = tmp_size;
            m_local_data = carb::cpp::bit_cast<local_data>(adata);
        }
        else if (!is_local())
        {
            ...
            auto adata = carb::cpp::bit_cast<allocated_data>(m_local_data);
            pointer tmp_ptr = adata.m_ptr;
            size_type tmp_size = adata.m_size;
...
constexpr string::size_type string::capacity() const noexcept
{
    ...
    {
        return carb::cpp::bit_cast<allocated_data>(m_local_data).m_capacity;
    }
}
...
inline string::iterator string::insert(const_iterator p, value_type c)
{
    ...
    size_type pos = (size_type)std::distance(cbegin(), p);
...
{
    return insert_internal(p, first, last, typename std::iterator_traits<InputIterator>::iterator_category{});
}
...
inline string& string::insert(size_type pos, const carb::cpp::string_view& sv)
{
    return insert_internal(pos, sv.data(), sv.size());
}

inline string& string::insert(size_type pos1, const carb::cpp::string_view& sv, size_type pos2, size_type n)
{
...
constexpr string::iterator string::erase(const_iterator pos)
{
    ...
    size_type start = (size_type)(pos - cbegin());
    if (start >= size())
    {
        return end();
    }
...
constexpr string::iterator string::erase(const_iterator first, const_iterator last)
{
    ...
    size_type pos = (size_type)(first - cbegin());
    ...
    {
        erase(pos, (size_type)(last - first));
    }
...
constexpr void string::pop_back()
{
    if (empty())
    {
        CARBLOCAL_THROW(std::runtime_error, "string::pop_back called on empty string");
    }
    erase(size() - 1, 1);
}
...
{
    return append_internal(first, last, typename std::iterator_traits<InputIterator>::iterator_category{});
}
...
inline string& string::append(const carb::cpp::string_view& sv)
{
    return append_internal(sv.data(), sv.size());
}

inline string& string::append(const carb::cpp::string_view& sv, size_type pos, size_type n)
{
    range_check(pos, sv.size(), "string::append");
    size_type substr_size = ::carb_min(n, (sv.size() - pos));

    CARB_IGNOREWARNING_GNUC_WITH_PUSH("-Warray-bounds") // array subscript is above array bounds
...
CARBLOCAL_STDSTRING_CONSTEXPR inline int string::compare(const std::string& str) const noexcept
{
    return compare_internal(get_pointer(0), str.data(), size(), str.size());
}

CARBLOCAL_STDSTRING_CONSTEXPR inline int string::compare(size_type pos1, size_type n1, const std::string& str) const
{
    range_check(pos1, size(), "string::compare");
...
CARBLOCAL_STDSTRING_CONSTEXPR inline int string::compare(
...
constexpr int string::compare(const carb::cpp::string_view& sv) const noexcept
{
    return compare_internal(get_pointer(0), sv.data(), size(), sv.size());
}

constexpr int string::compare(size_type pos1, size_type n1, const carb::cpp::string_view& sv) const
{
    range_check(pos1, size(), "string::compare");
...
constexpr int string::compare(size_type pos1, size_type n1, const carb::cpp::string_view& sv, size_type pos2, size_type n2) const
{
    range_check(pos1, size(), "string::compare");
...
constexpr bool string::starts_with(carb::cpp::string_view sv) const noexcept
{
    if (size() >= sv.size())
    {
...
constexpr bool string::ends_with(carb::cpp::string_view sv) const noexcept
{
    if (size() >= sv.size())
    {
...
constexpr bool string::contains(carb::cpp::string_view sv) const noexcept
{
    return find(sv) != npos;
}
...
string& string::replace(const_iterator i1, const_iterator i2, InputIterator j1, InputIterator j2)
{
    ...
    return replace_internal(
        i1, i2, j1, j2,
        carb::cpp::disjunction<std::is_same<InputIterator, iterator>, std::is_same<InputIterator, const_iterator>>{});
}

template <class InputIterator>
string& string::replace_internal(const_iterator i1, const_iterator i2, InputIterator j1, InputIterator j2, std::false_type)
{
    size_type const pos = (size_type)std::distance(cbegin(), i1);
    size_type const n1 = (size_type)std::distance(i1, i2);

    // Use a temp for simplicity since InputIterator cannot be iterated more than once.
    string tmp(j1, j2);
    return replace(pos, n1, tmp.data(), tmp.size());
}

inline string& string::replace_internal(
    const_iterator i1, const_iterator i2, const_iterator j1, const_iterator j2, std::true_type)
{
    size_type const pos = (size_type)std::distance(cbegin(), i1);
    size_type const n1 = (size_type)std::distance(i1, i2);

    return replace(pos, n1, j1.operator->(), (size_type)std::distance(j1, j2));
}
...
inline string& string::replace(size_type pos1, size_type n1, const carb::cpp::string_view& sv)
{
    return replace(pos1, n1, sv.data(), sv.size());
}

inline string& string::replace(size_type pos1, size_type n1, const carb::cpp::string_view& sv, size_type pos2, size_type n2)
{
...
{
    range_check(i1, i2, "string::replace");
    size_type pos = (size_type)std::distance(cbegin(), i1);
    size_type n1 = (size_type)std::distance(i1, i2);
...
inline string& string::replace(const_iterator i1, const_iterator i2, const carb::cpp::string_view& sv)
{
    range_check(i1, i2, "string::replace");
    size_type pos = (size_type)std::distance(cbegin(), i1);
    size_type n1 = (size_type)std::distance(i1, i2);
...
constexpr string::size_type string::find(const value_type* s, size_type pos, size_type n) const
{
    ...
    while (remaining_length >= n)
    {
        ...
        if (traits_type::compare(search_start, s, n) == 0)
        {
            ...
            return (size_type)(search_start - get_pointer(0));
        }

        // Match was not found, update remaining length and try again
        ++search_start;
        remaining_length = (size_type)(last_char - search_start);
    }
...
constexpr string::size_type string::find(value_type c, size_type pos) const noexcept
{
    ...
    const value_type* const p =
        (is_local() ? m_local_data.data() : carb::cpp::bit_cast<allocated_data>(m_local_data).m_ptr);

    const value_type* location = traits_type::find(p + pos, s - pos, c);
    if (location != nullptr)
    {
        return (size_type)(location - p);
    }
    return npos;
}

CARBLOCAL_STDSTRING_CONSTEXPR inline string::size_type string::find(const std::string& str, size_type pos) const noexcept
{
    return find(str.data(), pos, str.size());
}

constexpr string::size_type string::find(const carb::cpp::string_view& sv, size_type pos) const noexcept
{
    return find(sv.data(), pos, sv.size());
}
...
CARBLOCAL_STDSTRING_CONSTEXPR inline string::size_type string::rfind(const std::string& str, size_type pos) const noexcept
{
    return rfind(str.data(), pos, str.size());
}

constexpr string::size_type string::rfind(const carb::cpp::string_view& sv, size_type pos) const noexcept
{
...
CARBLOCAL_STDSTRING_CONSTEXPR inline string::size_type string::find_first_of(const std::string& str,
                                                                             size_type pos) const noexcept
{
    return find_first_of(str.data(), pos, str.size());
}

constexpr string::size_type string::find_first_of(const carb::cpp::string_view& sv, size_type pos) const noexcept
{
...
CARBLOCAL_STDSTRING_CONSTEXPR inline string::size_type string::find_last_of(const std::string& str,
                                                                            size_type pos) const noexcept
{
    return find_last_of(str.data(), pos, str.size());
}

constexpr string::size_type string::find_last_of(const carb::cpp::string_view& sv, size_type pos) const noexcept
{
...
CARBLOCAL_STDSTRING_CONSTEXPR inline string::size_type string::find_first_not_of(const std::string& str,
                                                                                 size_type pos) const noexcept
{
    return find_first_not_of(str.data(), pos, str.size());
}

constexpr string::size_type string::find_first_not_of(const carb::cpp::string_view& sv, size_type pos) const noexcept
{
...
CARBLOCAL_STDSTRING_CONSTEXPR inline string::size_type string::find_last_not_of(const std::string& str,
                                                                                size_type pos) const noexcept
{
    return find_last_not_of(str.data(), pos, str.size());
}

constexpr string::size_type string::find_last_not_of(const carb::cpp::string_view& sv, size_type pos) const noexcept
{
...
{
    return m_local_data.data()[kSMALL_SIZE_OFFSET] != kSTRING_IS_ALLOCATED;
}

CARBLOCAL_UNION_CONSTEXPR inline void string::set_local(size_type new_size) noexcept
{
    CARB_ASSERT(new_size < kSMALL_STRING_SIZE, "Local size must be less than kSMALL_STRING_SIZE");
    m_local_data.data()[kSMALL_SIZE_OFFSET] = static_cast<char>(kSMALL_STRING_SIZE - new_size - 1);
}

CARBLOCAL_UNION_CONSTEXPR inline void string::set_allocated() noexcept
{
    m_local_data.data()[kSMALL_SIZE_OFFSET] = kSTRING_IS_ALLOCATED;
}
...
constexpr string::reference string::get_reference(size_type pos) noexcept
{
    if (is_local())
    {
        // ensure GCC that reading outside the array bounds is UB
        CARB_GNUC11_ONLY(CARB_ASSUME(pos < kSMALL_STRING_SIZE);)
        return m_local_data.data()[pos];
    }
    else
    {
        return carb::cpp::bit_cast<allocated_data>(m_local_data).m_ptr[pos];
    }
}
...
CARBLOCAL_UNION_CONSTEXPR inline void string::set_empty() noexcept
{
    m_local_data = {};
    m_local_data.data()[kSMALL_SIZE_OFFSET] = kSMALL_STRING_SIZE - 1;
}
...
constexpr void string::range_check(size_type pos, size_type size, const char* function) const
{
    ...
    {
        CARBLOCAL_THROW(std::out_of_range, (std::string(function) + ": Provided pos " + std::to_string(pos) +
...
constexpr void string::range_check(const_iterator pos, const char* function) const
{
    ...
    {
        CARBLOCAL_THROW(std::out_of_range,
                        (std::string(function) + ": Provided iterator comes before the start of the string").c_str());
    }
    else if (CARB_UNLIKELY(pos > cend()))
    {
        CARBLOCAL_THROW(std::out_of_range,
                        (std::string(function) + ": Provided iterator comes after the end of the string").c_str());
    }
}
...
constexpr void string::range_check(const_iterator first, const_iterator last, const char* function) const
{
    ...
    {
        CARBLOCAL_THROW(std::out_of_range, (std::string(function) + ": Iterator range is not valid").c_str());
    }
}
...
constexpr string::size_type string::length_check(size_type current, size_type n, const char* function) const
{
    ...
    {
        CARBLOCAL_THROW(std::length_error, (std::string(function) + ": Adding " + std::to_string(n) +
...
constexpr void string::set_size(size_type new_size) noexcept
{
    if (is_local())
    {
        CARB_ASSERT(new_size < kSMALL_STRING_SIZE);
        // GCC requires this `if` statement to understand that `new_size` is within the small string size.
        // Otherwise -Warray-bounds and/or -Wstringop-overflow are issued depending on GCC version.
        CARB_NOT_MSC(CARB_ASSUME(new_size < kSMALL_STRING_SIZE);)
        m_local_data.data()[new_size] = value_type();
        m_local_data.data()[kSMALL_SIZE_OFFSET] = static_cast<char>(kSMALL_STRING_SIZE - new_size - 1);
    }
    else
    {
        auto adata = carb::cpp::bit_cast<allocated_data>(m_local_data);
        adata.m_ptr[new_size] = value_type();
        adata.m_size = new_size;
        m_local_data = carb::cpp::bit_cast<local_data>(adata);
    }
}
...
inline void string::overlap_check(const_pointer s) const
{
    if (CARB_LIKELY(!overlaps_this_string(s)))
        return;
    CARBLOCAL_THROW(std::runtime_error, "string may not overlap");
}
...
inline string::size_type string::vsnprintf_check(char* buffer, size_type buffer_size, const char* format, va_list args)
{
    ...
    if (fmt_size < 0)
    {
        CARBLOCAL_THROW(std::runtime_error, "vsnprintf failed");
    }
...
string::size_type string::snprintf_check(char* buffer, size_type buffer_size, const char* format, Args&&... args)
{
    ...
    if (fmt_size < 0)
    {
        CARBLOCAL_THROW(std::runtime_error, "snprintf failed");
    }
    return size_type(fmt_size);
}

inline string::pointer string::allocate_if_necessary(size_type size)
{
    if (should_allocate(size))
    {
        auto adata = carb::cpp::bit_cast<allocated_data>(m_local_data);
        adata.m_ptr = static_cast<char*>(carb::allocate(size + 1));
        adata.m_capacity = size;
        m_local_data = carb::cpp::bit_cast<local_data>(adata);
        set_allocated();
    }
    return get_pointer(0);
...
{
    // gcc 11: `__builtin_memcpy` forming offset [X, Y] is out of the bounds [0, X]
    CARB_GNUC11_ONLY(CARB_IGNOREWARNING_GNUC_WITH_PUSH("-Warray-bounds"))
    std::memcpy(allocate_if_necessary(size), src, size);
    CARB_GNUC11_ONLY(CARB_IGNOREWARNING_GNUC_POP)
    set_size(size);
}

template <typename InputIterator>
void string::initialize(InputIterator begin, InputIterator end, std::input_iterator_tag)
{
    // InputIterator cannot be measured; it can only be iterated once
    CARBLOCAL_TRY_BEGIN
    {
        for (; begin != end; ++begin)
            push_back(*begin);
...
void string::initialize(ForwardIterator begin, ForwardIterator end, std::forward_iterator_tag)
{
    auto const size = (size_type)std::distance(begin, end);
    for (auto ptr = allocate_if_necessary(size); begin != end; ++begin, ++ptr)
    {
...
inline void string::dispose()
{
    if (!is_local())
    {
        carb::deallocate(carb::cpp::bit_cast<allocated_data>(m_local_data).m_ptr);
        set_empty();
    }
}
...
inline string::pointer string::grow_buffer_to(size_type new_capacity)
{
    if (new_capacity > capacity())
    {
        ...
        new_buffer[cur_size] = value_type();

        // Destroy the old buffer
        dispose();
        set_allocated();
        auto adata = carb::cpp::bit_cast<allocated_data>(m_local_data);
        adata.m_ptr = new_buffer;
        adata.m_capacity = new_capacity;
        adata.m_size = cur_size;
        m_local_data = carb::cpp::bit_cast<local_data>(adata);
        return new_buffer;
    }
    return get_pointer(0);
    ...
    size_type new_size, const_pointer p1, size_type s1, const_pointer p2, size_type s2, const_pointer p3, size_type s3)
{
    ...
    new_buffer[new_size] = value_type();
...
void string::grow_buffer_and_append(size_type new_size, ForwardIterator first, ForwardIterator last)
{
    size_type new_capacity = new_size;
    pointer new_buffer = allocate_buffer(capacity(), new_capacity);
...
template <typename InputIterator>
string& string::assign_internal(InputIterator begin, InputIterator end, std::input_iterator_tag)
{
    // InputIterator cannot be measured; it can only be iterated once. Create a temporary string from the iterator.
    // If it throws, we have not modified *this.
...
template <typename ForwardIterator>
string& string::assign_internal(ForwardIterator begin, ForwardIterator end, std::forward_iterator_tag)
{
    size_t const new_size = length_check(0, (size_type)std::distance(begin, end), "string::assign");
...
inline string& string::insert_internal(size_type pos, value_type c, size_type n)
{
    ...
    pointer copy_start = grow_buffer_to(new_size) + pos;

    size_type to_copy = size() - pos;
    pointer copy_dest = copy_start + n;
    // `__builtin_memmove` offset Y is out of the bounds [0, X] ???
    CARB_GNUC11_ONLY(CARB_ASSUME(copy_start >= get_pointer(0) && copy_start < (get_pointer(0) + size()));)
    std::memmove(copy_dest, copy_start, to_copy);
...
template <class InputIterator>
string::iterator string::insert_internal(const_iterator p, InputIterator begin, InputIterator end, std::input_iterator_tag)
{
    range_check(p, "string::insert");
    size_type const pos = (size_type)std::distance(cbegin(), p);
    ...
    CARBLOCAL_CATCH_ALL
    {
        // Exception thrown; revert back to previous length and re-throw
        set_size(prev_size);
        CARBLOCAL_RETHROW;
    }

    // Move the new elements into position
    std::rotate(get_pointer(pos), get_pointer(prev_size), get_pointer(size()));
    return iterator(get_pointer(pos));
}

template <class ForwardIterator>
string::iterator string::insert_internal(const_iterator p,
                                         ...
                                         std::forward_iterator_tag)
{
    ...
    size_type const n = (size_type)std::distance(first, last);
    length_check(size(), n, "string::insert");
...
template <class InputIterator>
string& string::append_internal(InputIterator begin, InputIterator end, std::input_iterator_tag)
{
    // InputIterator cannot be measured; it can only be iterated once.
    auto const prev_size = size();
    CARBLOCAL_TRY_BEGIN
    ...
    CARBLOCAL_CATCH_ALL
    {
        // Exception caught, revert to previous size and re-throw
        set_size(prev_size);
        CARBLOCAL_RETHROW;
    }
...
template <class ForwardIterator>
string& string::append_internal(ForwardIterator first, ForwardIterator last, std::forward_iterator_tag)
{
    size_type const new_size = length_check(size(), (size_type)std::distance(first, last), "string::append");
...
CARBLOCAL_STDSTRING_CONSTEXPR inline bool operator==(const string& lhs, const std::string& rhs) noexcept
{
    return lhs.compare(rhs) == 0;
}

CARBLOCAL_STDSTRING_CONSTEXPR inline bool operator==(const std::string& lhs, const string& rhs) noexcept
{
    return rhs.compare(lhs) == 0;
}
...
#include "../carb/detail/PopBadMacros.h"
#undef CARB_GNUC11_ONLY
