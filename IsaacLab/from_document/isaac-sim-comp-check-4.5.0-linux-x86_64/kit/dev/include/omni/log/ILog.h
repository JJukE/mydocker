...
#define OMNI_LOG_WRITE(channelOrFormat_, level_, ...)                                                                  \
    OMNI_LOG_WRITE_EX(channelOrFormat_, level_, __FILE__, __func__, __LINE__, ##__VA_ARGS__)

#ifndef DOXYGEN_SHOULD_SKIP_THIS
#    define OMNI_LOG_WRITE_EX(channelOrFormat_, level_, file_, func_, line_, ...)                                      \
        do                                                                                                             \
        {                                                                                                              \
            ...
            {                                                                                                          \
                ...
                if (log_)                                                                                              \
                {                                                                                                      \
                    omni::log::detail::writeLog(                                                                       \
                        OMNI_LOG_DEFAULT_CHANNEL, channelOrFormat_, log_, level_, file_, func_, line_, ##__VA_ARGS__); \
                }                                                                                                      \
            }                                                                                                          \
        } while (0)
#endif
...
void writeLog(const LogChannelData& /*ignore*/,
              ...
              ArgList&&... args)
{
    log->logf(channel.name, level, omniGetModuleFilename(), filename, function, (uint32_t)line, fmt,
...
inline void addModulesChannels()
{
    ...
    if (log)
    {
        hasModuleLogChannelsRegistered() = true;
...
inline void removeModulesChannels()
{
    ...
    if (log)
    {
        hasModuleLogChannelsRegistered() = false;
        for (auto channel = getModuleLogChannels(); channel; channel = channel->next)
        {
...
public:
    LogChannelRegistrar(LogChannelData* data, bool add) noexcept
    {
        if (add)
        {
            ...
            // OMPE-9580: If registration has already been called, manually register this channel.
            if (hasModuleLogChannelsRegistered())
            {
                auto log = omniGetLogWithoutAcquire();
                if (log != nullptr)
                {
                    log->addChannel(data->name, reinterpret_cast<Level*>(&data->level), data->description);
                }
            }
        }
    }
