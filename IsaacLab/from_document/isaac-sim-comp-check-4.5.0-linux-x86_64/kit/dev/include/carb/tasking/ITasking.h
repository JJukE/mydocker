...
#define carb_tasking_ITasking_latest CARB_HEXVERSION(2, 5)
#ifndef carb_tasking_ITasking
//! Default version of ITasking interface to use
#    define carb_tasking_ITasking CARB_HEXVERSION(2, 4)
#endif
...
struct ITasking
{
    ...
    // 2.5 - Templated applyRange
    CARB_PLUGIN_INTERFACE_EX("carb::tasking::ITasking", carb_tasking_ITasking_latest, carb_tasking_ITasking)
...
#if CARB_VERSION_ATLEAST(carb_tasking_ITasking, 2, 5)
    //! @private
    detail::IApplyRange&(CARB_ABI* internalGetIApplyRange)(uint32_t ver);
#endif
     ...
     * @param f An invocable object (i.e. functor, lambda, [member] function ptr) that optionally returns a value.
     ...
     *
     * @note This function does not return until @p f has been called (and returned) on every index from `[0..range)`.
     ...
     * @param f An invocable object (i.e. functor, lambda, [member] function ptr) that is repeatedly called until
    ...
    /**
     * Processes a range from `[0..range)` calling an invocable for batches of indexes, potentially from different
     * threads.
     *
     ...
     *
     * @param range The number of times to call @p f.
     * @param batchHint A recommended batch size to use for the batches passed to invocations of @p f.
     * @param f An invocable object (i.e. functor, lambda, [member] function ptr) that is repeatedly called until
     *   all indexes in `[0..range)` have been processed, potentially from different threads. It is invoked with
     *   parameters `f(args..., startIndex, endIndex)` where `[startIndex..endIndex)` is the range of indexes that must
     *   be processed by that invocation of `f`. Note that `endIndex` is a past-the-end index and must not actually be
     *   processed by an invocation of `f`. Any return value of `f` is ignored. If possible, the batch size
     *   (i.e. `endIndex - startIndex`) will be around @p batchHint, but `endIndex` will not exceed @p range. The batch
     *   size may not be the same for all invocations of @p f.
     * @param args Arguments to pass to @p f
     */
    template <class Callable, class... Args>
...
#if CARB_VERSION_ATLEAST(carb_tasking_ITasking, 2, 5)
    /**
     * Applies a reduction body across a given range. Colloquially known as map/reduce.
     *
     * The reduction body must have the following elements:
     * * A splitting constructor of the signature `Body(Body& other, carb::tasking::Split_t)`. This constructor is
     *   called when a worker thread is available to split work off to be processed in parallel. `*this` is the new
     *   parallel work and `other` is the work in progress that is being split.
     * * A `operator()(size_t begin, size_t end)` function that will be called for a portion of `[0..range)` represented
     *   as `[begin..end)`. The `end` index is a past-the-end index and must not be processed. `end - begin` is
     *   guaranteed to be non-zero. Each instance of `Body` will have `operator()` called at least once and never in
     *   parallel. This is the 'map' portion of the map/reduce algorithm. Since this function may be called multiple
     *   times serially for an instance, it is important to retain results from previous calls.
     * * A `reduce(Body& other)` function that is called to incorporate the results from `other` into `*this`. This
     *   function is never called in parallel for a given instance, so synchronization to members is not required. This
     *   function is only called if the workload was previously split by invoking the *splitting constructor*.
     *
     * @note This function does not return until the map/reduce algorithm is complete over `[0..range)`.
     * @note Calling this function recursively will automatically scale down the parallelism in order to not overburden
     *   the system.
     *
     * See the @rstref{additional documentation <tasking-parallel-reduce>} for `parallelReduce`.
     *
     * @tparam Body The reduction body as described above.
     * @param range The range of indexes.
     * @param batchHint A recommended batch size to use for the batches passed to `Body::operator()` invocations.
     * @param body The reduction body.
     */
    template <class Body>
    void parallelReduce(size_t range, size_t batchHint, Body& body);
#endif
};
