...
public:
    ...
    bool open(const OpenToken& openToken, AccessMode access = AccessMode::eDefault)
    {
        ...
        {
            CARB_LOG_ERROR("failed to open a file mapping object with the name '%s' {error = %" PRIdword "}",
                           token->name, GetLastError());
            free(token);
            return false;
        }
#elif CARB_POSIX
        ...
        handle.handleFd = shm_open(mappingName.c_str(), (int)getAccessModeFlags(access, FlagType::eFileFlags), 0);
    ...
    protected:
        ...
        bool map(SharedHandle handle, size_t offset, size_t size, AccessMode access, size_t allocGran)
        {
            ...
            {
                CARB_LOG_ERROR(
                    "failed to map %zu bytes from offset %zu {error = %" PRIdword "}", size, offset, GetLastError());
                return false;
            }
#elif CARB_POSIX
            CARB_UNUSED(allocGran);
            m_pageOffset = 0;
            mapPtr = mmap(nullptr, size, (int)getAccessModeFlags(access, FlagType::ePageFlags), MAP_SHARED,
                          handle.handleFd, (off64_t)offset);
        ...
        void unmap()
        {
            ...
            if (UnmapViewOfFile(m_address) == CARBWIN_FALSE)
                CARB_LOG_ERROR("failed to unmap the region at %p {error = %" PRIdword "}", m_address, GetLastError());
#elif CARB_POSIX
...
#pragma pack(pop)
    ...
    std::string getPlatformMappingName(const char* name, size_t maxLength = 0)
    {
        ...
        {
            fname.erase(fname.begin() + (std::string::difference_type)maxLength, fname.end());
        }
    ...
    Result createAndOrOpen(const char* name, size_t size, uint32_t flags, bool tryOpen, bool tryCreate)
    {
        ...
        if (handle.handleWin32 == nullptr || (!tryOpen && (GetLastError() == CARBWIN_ERROR_ALREADY_EXISTS)))
        {
            if (!quiet)
                CARB_LOG_ERROR(
                    "failed to create and/or open a file mapping object with the name '%s' {error = %" PRIdword "}",
                    name, GetLastError());
        ...
        if (handle.handleFd == -1)
        {
            ...
            if (size > size_t(statbuf.st_size) && ftruncate(handle.handleFd, (off64_t)size) != 0)
            {
                if (!quiet)
        ...
        else if (ftruncate(handle.handleFd, (off64_t)size) != 0)
        {
            if (!quiet)
