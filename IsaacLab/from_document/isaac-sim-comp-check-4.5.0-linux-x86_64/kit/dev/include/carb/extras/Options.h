...
public:
     ...
     *
     *  @returns The value converted into the requested format if possible.  This returned string
     *           must be copied if it needs to persist beyond the life of this object.
     */
...
private:
    ...
    std::string m_string; ///< The value when storing a string.
 ...
 *                      If this value contains a string, it must be copied if it needs to
 *                      persist beyond this call.  The string that holds the value will be
 *                      destroyed upon return from this callback.
...
/** Special failure value for getArgString() indicating that the given argument index was out
 *  of range of the argument list.  No processing will occur on the requested argument.
 */
...
{
    char* endp = nullptr;
    *value = (T)strtoull(string, &endp, 10);
...
/** Tests if the return value from getArgString() was a failure code.
 *
 *  @param[in] result   The return value from getArgString() to check for a failure.
 *  @returns `true` if the result code was one of the @a kArgFail* error codes.  Returns `false`
 ...
 *  @param[in] argIndex The zero-based index of the argument to parse.  This must be less than or
 *                      equal to @p argc and greater than or equal to zero.
 *  @param[out] value   Receives the value's string.
 *  @returns The number of consumed arguments beyond the argument that was originally requested to
 *           be processed.  Since an option's value could either be part of the given argument
 *           (ie: the name followed by '=') or specified in the following argument, this may be 0
 *           or 1.
 *  @returns A negative error code if a parsing error occurred or the argument could not be
