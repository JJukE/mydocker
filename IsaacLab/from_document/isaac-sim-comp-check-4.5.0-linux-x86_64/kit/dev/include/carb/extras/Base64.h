...
public:
    ...
    size_t encode(const void* buffer, size_t size, char* output, size_t maxOut)
    {
        ...
        {
            data = (uint32_t)((ptr[i + 0] << 16) | (ptr[i + 1] << 8) | (ptr[i + 2] << 0));
        ...
        switch (extra)
        {
            ...
            case 2:
                data = (uint32_t)((ptr[stop + 0] << 16) | (ptr[stop + 1] << 8));
    ...
    size_t decode(const char* buffer, size_t size, void* output, size_t maxOut)
    {
        ...
        {
            data = (uint32_t)((m_decode[ptr[i + 0]] << 18) | (m_decode[ptr[i + 1]] << 12) |
                              (m_decode[ptr[i + 2]] << 6) | (m_decode[ptr[i + 3]] << 0));
        ...
        switch (extra)
        {
            ...
            // output byte.
            case 2:
                data = (uint32_t)((m_decode[ptr[stop + 0]] << 18) | (m_decode[ptr[stop + 1]] << 12));
...
private:
    ...
    void initCodec(uint8_t char62, uint8_t char63, uint8_t padding)
    {
        ...
        m_encode[62] = (char)char62;
        m_encode[63] = (char)char63;

        // generate the decoding map as a the reverse of the encoding table.
        memset(m_decode, 0, sizeof(m_decode));

        for (size_t i = 0; i < 64; i++)
            m_decode[static_cast<size_t>(m_encode[i])] = (unsigned char)(i & 0xff);
