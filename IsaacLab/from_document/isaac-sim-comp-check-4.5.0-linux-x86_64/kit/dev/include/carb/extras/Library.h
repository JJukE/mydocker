 ...
 *
 * \warning On some older platforms this flag may not be supported. Providing it may cause the library to fail to open.
 *
 * Calls to \ref unloadLibrary() will no longer have any effect once the library is opened with this flag.
 */
...
inline std::string createLibraryNameForModule(const char* baseName)
{
    ...
    if (sep[0] != nullptr)
    {
        pathLen = (size_t)((sep[0] - baseName) + 1);
        name = sep[0] + 1;
        len += pathLen;
    }
 ...
 *                          it does not increment the reference count, but on Linux it increments the
...
inline void unloadLibrary(LibraryHandle libraryHandle)
{
    if (libraryHandle)
    {
        ...
        if (!::FreeLibrary(libraryHandle))
        {
            ...
            CARB_LOG_WARN("FreeLibrary for handle %p failed with error: %" PRIdword "/%s", libraryHandle, err,
