...
CARB_IGNOREWARNING_GNUC_WITH_PUSH("-Wconversion") // conversion to 'unsigned int:31' from 'uint32_t' may alter its value
struct Rec
{
    Rec* m_next;
    uint32_t m_stringId;
...
inline bool uncasedEqual(const char* str1, const char* str2, size_t len)
{
    ...
    while (str1 != str1end)
    {
        if (carb::tolower((unsigned char)*(str1++)) != carb::tolower((unsigned char)*(str2++)))
            return false;
    }
    return true;
}
...
inline int uncasedCompare(const char* str1, size_t len1, const char* str2, size_t len2)
{
    ...
    {
        signed char c1 = (signed char)carb::tolower((unsigned char)*str1);
        signed char c2 = (signed char)carb::tolower((unsigned char)*str2);
        int val = c1 - c2;
        if (val != 0)
            return val;
    }
...
public:
    ...
    size_t getHash(uint32_t stringId)
    {
        ...
        carb::cpp::atomic_ref<size_t> hashRef(rec->m_hash);
...
private:
    static bool validate(Data* pData)
    {
        ...
        // so we're going to put a time limit on it. NOTE: This used to use cpp::atomic::wait_for(), but this does not
        ...
        auto timeout = cpp::detail::absTime<std::chrono::steady_clock>(std::chrono::seconds(5));
    ...
    void init()
    {
        ...
        m_data->AllocSize = (m_data->AllocSize + allocGranularity - 1) & (size_t)(-ptrdiff_t(allocGranularity));
        ...
        sizeNeeded = (sizeNeeded + allocGranularity - 1) & (size_t)(-ptrdiff_t(allocGranularity));
...
#undef EMPTY_ENTRY
        ...
        // NOTE: Older versions of RString used cpp::atomic::wait and notify_all(), but this doesn't work properly
    ...
    {
        // Static checks
        static_assert(std::is_standard_layout<decltype(m_version)>::value, "Only standard layout members are allowed.");
        static_assert(
            std::is_standard_layout<decltype(m_initializedByMe)>::value, "Only standard layout members are allowed.");
        static_assert(std::is_standard_layout<decltype(m_unused)>::value, "Only standard layout members are allowed.");
        static_assert(std::is_standard_layout<decltype(m_next)>::value, "Only standard layout members are allowed.");
        static_assert(std::is_standard_layout<decltype(m_prev)>::value, "Only standard layout members are allowed.");
        static_assert(
            std::is_standard_layout<decltype(m_visualizer)>::value, "Only standard layout members are allowed.");
        static_assert(std::is_standard_layout<decltype(m_shm)>::value, "Only standard layout members are allowed.");
        static_assert(std::is_standard_layout<decltype(m_view)>::value, "Only standard layout members are allowed.");
        static_assert(std::is_standard_layout<decltype(m_data)>::value, "Only standard layout members are allowed.");
        ...
        extras::detail::NamedSemaphore sema{ extras::detail::getGlobalSemaphoreName() };
        std::unique_lock<extras::detail::NamedSemaphore> guard(sema);
        ...
        {
            // Wait until initialized is non-zero. NOTE: Previously this used a cpp::atomic::wait(), but this will not
    ...
    void lockMutex()
    {
        ...
        CARB_UNLIKELY_IF(
            !m_data->lock.compare_exchange_strong(state, Locked, std::memory_order_acquire, std::memory_order_relaxed))
        {
            if (state == LockedMaybeWaiting)
            {
                // Failed to lock and need to wait
                // NOTE: This used to use cpp::atomic::wait(), but this does not work properly since each binary maps
    ...
    void unlockMutex()
    {
        // Unlock the mutex. Older versions of RString used cpp::atomic::notify_one(), but this doesn't work
    ...
    Rec* hashTableFind(const char* str, size_t len, bool uncased, size_t const uncasedHash) const
    {
        ...
        // store in findOrAdd().
        Rec* rec = carb::cpp::atomic_ref<Rec*>(m_data->hashTableBuckets[uncasedHash & (m_data->NumHashBuckets - 1)])
    ...
    uint32_t findOrAdd(eRString enumVal, const char* str, size_t const len, bool uncased, RStringOp op)
    {
        ...
        sizeNeeded = (sizeNeeded + (alignof(Rec) - 1)) & (size_t)(-ptrdiff_t(alignof(Rec)));
        ...
        *ppRec = new (m_data->mem) Rec(pBucketHead, uint32_t(index), uint32_t(len), !authority, uncasedHash, str);
        ...
        carb::cpp::atomic_ref<Rec*>(pBucketHead).store(*ppRec, std::memory_order_release);
    ...
    carb::extras::SharedMemory::View* m_view;
    Data* m_data;
};

static_assert(std::is_standard_layout<Internals>::value, "Internals class must have a standard layout.");
