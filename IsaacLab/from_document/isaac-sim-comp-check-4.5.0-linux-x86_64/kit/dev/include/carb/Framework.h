...
struct PluginRegistrationDesc
{
    ...
    OnPluginStartupFn onPluginStartupFn = nullptr; //!< Can be `nullptr`.
    OnPluginShutdownFn onPluginShutdownFn = nullptr; //!< Can be `nullptr`.
    GetPluginDepsFn getPluginDepsFn = nullptr; //!< Can be `nullptr`.
    OnReloadDependencyFn onReloadDependencyFn = nullptr; //!< Can be `nullptr`.
    OnPluginPreStartupFn onPluginPreStartupFn = nullptr; //!< Can be `nullptr`.
    OnPluginPostShutdownFn onPluginPostShutdownFn = nullptr; //!< Can be `nullptr`.
    OnPluginRegisterExFn onPluginRegisterExFn = nullptr; //!< Can be `nullptr`.
...
CARB_ASSERT_INTEROP_SAFE(PluginRegistrationDesc);
...
CARB_IGNOREWARNING_CLANG_WITH_PUSH("-Wdeprecated-declarations") //'internalRealloc' is deprecated: Use carbReallocate()
                                                                // instead
...
struct Framework
{
    ...
    //!
    //! A plugin that adds release hooks for its own interfaces will have release hooks called \a after termination
    //! (that is, after \ref carbOnPluginShutdown has been called). Release hooks added by other clients (application or
    //! plugins) will be called \a before \ref carbOnPluginShutdown is called for a plugin.
    //!
    ...
    // NOTE: Superseded by internalAddReleaseHook2 but maintained for backwards compatibility
    //! @private
    bool(CARB_ABI* internalAddReleaseHook)(void* iface, ReleaseHookFn fn, void* user);
    ...
    //! @private
    bool(CARB_ABI* internalAddReleaseHook2)(const char* client, void* iface, ReleaseHookFn fn, void* user);
};
} // namespace carb
...
{
    CARB_UNUSED(T::getInterfaceDesc()); // Compile-time check that the type is a plugin interface.
    const char* clientName = g_needToCall_CARB_GLOBALS_atGlobalScope ? g_carbClientName : nullptr;
...
inline bool Framework::addReleaseHook(void* iface, ReleaseHookFn fn, void* user)
{
    const char* clientName = g_needToCall_CARB_GLOBALS_atGlobalScope ? g_carbClientName : nullptr;
    if (this->internalAddReleaseHook2)
        return this->internalAddReleaseHook2(clientName, iface, fn, user);
    else
        return this->internalAddReleaseHook(iface, fn, user);
}
