...
//! API functions. Update: Deprecated. Define left for compatibility.
#define CARB_ABI
...
#    if CARB_TOOLCHAIN_CLANG
//! Defined as `[[clang::fallthrough]]` if the current compiler supports C++17; empty otherwise.
#        define CARB_FALLTHROUGH [[clang::fallthrough]]
#    else
    ...
    const char* condition, const char* file, const char* func, int32_t line, const char* fmt = nullptr, ...)
{
    if (fmt != nullptr)
    {
        ...
        va_end(args);
        fprintf(stderr, "%s:%" PRId32 " (%s):: Assertion (%s) failed: %s\n", file, line, func, condition, message);
    }
    else
...
#if (CARB_HAS_CPP17 && defined(__cpp_lib_hardware_interference_size))
...
{
    return n > 0 ? fnv1aHash(str + 1, n - 1, (hash ^ (uint64_t)(*str)) * kFnvPrime) : hash;
}
 ...
 * Executing CARB_ASSUME(b) with `b == false` results in undefined behavior.
 */
