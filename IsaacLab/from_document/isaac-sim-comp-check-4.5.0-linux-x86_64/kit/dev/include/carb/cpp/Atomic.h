...
struct atomic_flag
{
    ...
    // C++20 features
    CARB_NODISCARD bool test(const std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return m_flag.load(order) != 0;
    }
    CARB_NODISCARD bool test(const std::memory_order order = std::memory_order_seq_cst) const volatile noexcept
    {
