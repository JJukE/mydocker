...
CARB_IGNOREWARNING_CLANG_WITH_PUSH("-Wdeprecated-declarations") // This function or variable may be unsafe. To disable
...
inline void handlePythonError(const py::error_already_set& e)
{
    // NOTE: GIL must be held
    try
    {
        // Call the Python exception hook
        py::module::import("sys").attr("excepthook")(e.type(), e.value(), e.trace());
    }
    catch (std::exception& ex)
    {
        OMNI_LOG_ERROR("While handling py::error_already_set(%s), another exception occurred: %s", e.what(), ex.what());
    }
    catch (...)
    {
        OMNI_LOG_ERROR("While handling py::error_already_set(%s), a non-std::exception was thrown", e.what());
    }
}
