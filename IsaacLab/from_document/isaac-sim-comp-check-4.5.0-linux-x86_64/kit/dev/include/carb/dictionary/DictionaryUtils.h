...
{
    auto deleter = [serializer](auto* s) { serializer->destroyStringBuffer(s); };
    std::unique_ptr<const char, decltype(deleter)> serializedString(
        serializer->createStringBufferFromDictionary(dictionary, serializerOptions), deleter);
    ...
    fs->writeFileChunk(sFile, serializedString.get(), strlen(serializedString.get()));
    fs->closeFile(sFile);
}
 ...
 *                  in as @p dict.
 *  @param[in] includeRoot  Set to `true` to include the root node's name.  This is the default
 *                  behavior.  Set to `false` to skip the root node's name.  The root node is
 *                  typically a node with a name that identifies the source of the dictionary
 *                  (ie: `<ISettings root>`, the name of the file a dictionary was loaded from,
 *                  etc).
...
inline void unsubscribeTreeFromAllEvents(IDictionary* dict, Item* item)
{
    ...
    walkDictionary(dict, WalkerMode::eIncludeRoot, item, 0u, unsubscribeItem, dict, getChildByIndexMutable);
}
