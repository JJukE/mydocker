...
inline void setPyObjectLocked(dictionary::IDictionary* idictionary, Item* baseItem, const char* path, const py::handle& value)
{
    auto createDict = [](dictionary::IDictionary* idictionary, Item* baseItem, const char* path, const py::handle& value) {
        ...
        for (auto& kv : valueDict)
        {
            ...
            setPyObjectLocked(idictionary, baseItem, subPath.c_str(), kv.second);
            empty = false;
        }
        if (empty)
        {
            auto item = idictionary->makeDictionaryAtPath(baseItem, path);
...
inline void setPyObject(dictionary::IDictionary* idictionary, Item* baseItem, const char* path, const py::handle& value)
{
    ...
    setPyObjectLocked(idictionary, baseItem, path, value);
}
        ...
        .def("__bool__",
             [](Item& self) {
                 ...
                 if (type == ItemType::eDictionary)
                     return getDictionary()->getItemChildCount(&self) > 0;
                 else
                     return getDictionary()->getAsBool(&self);
             },
             py::call_guard<py::gil_scoped_release>())
        .def("get",
