...
public:
    uint8_t* Allocate(size_t& size)
    {
        ...
        size_t const granularity = (size_t)getpagesize();
...
#    endif
        ...
        CARB_FATAL_UNLESS(ftruncate64(fd, (off64_t)size) != -1, "ftruncate failed: %d/%s", errno, strerror(errno));
...
private:
    ...
    {
        return reinterpret_cast<T*>((reinterpret_cast<size_t>(where) + align - 1) & (~align + 1));
    }
...
inline size_t RingBuffer::approx_used() const
{
    ...
    ptrdiff_t diff = (ptrdiff_t)(write - read);
...
inline void* RingBuffer::alloc(size_t bytes, size_t align)
{
    ...
    for (;;)
    {
        ...
        needed = (size_t)(baseNeeded + (size_t)padding);
    ...
    if (padding)
    {
        ...
        uint32_t size = (uint32_t)((size_t)padding - sizeof(Header));
        new (currentMem - sizeof(Header)) Header(size, size, Header::kPadding | Header::kCommitted);
        currentMem += padding;
    }
    ...
    {
        m_writeTail.wait(tail, std::memory_order_acquire);
...
{
    this_thread::atomic_fence_seq_cst();
    size_t readPtr = m_readPtr.load(std::memory_order_relaxed);
    size_t writePtr = m_writeTail.load(std::memory_order_relaxed);
...
{
    this_thread::atomic_fence_seq_cst();
    size_t origReadPtr = m_readPtr.load(std::memory_order_relaxed);
    size_t writePtr = m_writeTail.load(std::memory_order_relaxed);
...
inline void RingBuffer::waitForAllocatedData()
{
    ...
    while (ptrdiff_t(writePtr - readPtr) <= 0)
    {
        ...
        writePtr = m_writeTail.load(std::memory_order_acquire);
    }
}
...
inline void RingBuffer::waitForCommittedData()
{
    ...
    for (;;)
    {
        ...
        // Must be committed
        while (!(bytes & Header::kCommitted))
        {
            // Wait for commit
            header->bytes.wait(bytes, std::memory_order_relaxed);
            bytes = header->bytes.load(std::memory_order_acquire);
        }
