...
public:
     ...
     *
     * @param f An invocable object that accepts a `T*` parameter. Called for each item that was popped from the queue.
     */
    template <class Func>
    void forEach(Func&& f)
    {
    ...
    {
        return popSCWaitUntil(cpp::detail::absTime<std::chrono::steady_clock>(dur));
    }
    ...
    {
        return popMCWaitUntil(cpp::detail::absTime<std::chrono::steady_clock>(dur));
    }
    ...
    // Not really private, but should only be used in cases where the internals are well-understood.
    //! @private
    const std::atomic<T*>& head() const
    {
        return m_head;
    }
    ...
    // Not really private, but should only be used in cases where the internals are well-understood.
    //! @private
    const cpp::atomic<T*>& tail() const
    {
        return m_tail;
    }

private:
    CARB_VIZ std::atomic<T*> m_head; // doesn't require waiting, so `std::atomic` not `cpp::atomic`.
    CARB_VIZ cpp::atomic<T*> m_tail;
    static_assert(cpp::atomic<T*>::is_always_lock_free, "Requires lock-free atomics");
#if CARB_HAS_CPP17
    static_assert(std::atomic<T*>::is_always_lock_free, "Requires lock-free atomics");
#endif
