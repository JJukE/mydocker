...
#define carb_profiler_IProfiler_latest CARB_HEXVERSION(1, 5)
#ifndef carb_profiler_IProfiler
//! The default current version for carb::profiler::IProfiler
#    define carb_profiler_IProfiler CARB_HEXVERSION(1, 4)
#endif
...
struct IProfiler
{
    // 1.5: Added frameWithImage
    CARB_PLUGIN_INTERFACE_EX("carb::profiler::IProfiler", carb_profiler_IProfiler_latest, carb_profiler_IProfiler)
...
#if CARB_VERSION_ATLEAST(carb_profiler_IProfiler, 1, 5)
    /**
     * Inserts an anonymous frame marker along with an RGBA image (for profilers that support it).
     *
     * @param mask Event capture mask.
     * @param rgbaImageData pointer to the start of the image data (pixels must be in RGBA order, A is ignored).
     ...
     */
    void(CARB_ABI* frameWithImage)(const uint64_t mask,
                                   const void* rgbImageData,
                                   const uint32_t imageDataSizeInBytes,
                                   uint32_t width,
                                   uint32_t height);
#endif
};
